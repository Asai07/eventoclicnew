...
// --- DATOS DEL CATÁLOGO (Rutas locales en /public/catalogo/) ---
const catalogModels = [
    {
        id: 'm1',
        title: 'Nallely & Ricardo',
        category: 'bodas',
        tag: 'Boda',
...
export default function CatalogSection() {
    ...
    // LÓGICA DINÁMICA: Solo mostrar categorías que tengan al menos 1 modelo (o 'todos')
    const visibleCategories = allCategories.filter(cat =>
        cat.id === 'todos' || catalogModels.some(model => model.category === cat.id)
    );
    ...
    const whatsappNumber = "5218130550688";
                ...
                >
                    <AnimatePresence>
                        {filteredModels.map((model) => {
                            ...
                            const waMessage = encodeURIComponent(`Hola, me gustaría más información sobre el modelo: ${model.title}`);
                            const waLink = `https://wa.me/${whatsappNumber}?text=${waMessage}`;

                            // Placeholder para el link de la Demo
                            const demoLink = "#";
                            ...
                            return (
                                <motion.div
                                    ...
                                    key={model.id}
                                    className="group relative w-full aspect-[9/16] max-h-[600px] rounded-[2.5rem] overflow-hidden bg-zinc-100 shadow-xl shadow-zinc-200/50 cursor-default"
                                >
                                    ...
                                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-between p-6 md:p-8">
                                        ...
                                        <div className="flex flex-col translate-y-8 group-hover:translate-y-0 transition-transform duration-500 ease-[0.22,1,0.36,1]">
                                            ...
                                            <div className="flex flex-col gap-3">
                                                {/* CTA WhatsApp Original Dinámico */}
                                                <a
                                                    href={waLink}
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                                    className="w-full flex items-center justify-center gap-2 bg-[#2d5a27] hover:bg-[#1e3f1b] text-white py-4 rounded-full font-montserrat font-semibold text-sm transition-colors shadow-lg shadow-[#2d5a27]/30"
                                                >
                                                ...
                                                {/* CTA Ver demo (Placeholder) */}
                                                <a
                                                    href={demoLink}
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                                    className="w-full flex items-center justify-center gap-2 bg-[#cf655b] hover:bg-[#b5584f] text-white py-4 rounded-full font-montserrat font-semibold text-sm transition-colors shadow-lg shadow-[#cf655b]/30"
                                                >
                                                    Ver demo
                                                </a>
                                            </div>
